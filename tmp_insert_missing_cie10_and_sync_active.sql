-- Insert missing ICD-10 rows from CIE 10.xlsx and sync active flags.
-- Generated 2026-06-12. Source TSV: /Users/jorgeluisdevera/PhpstormProjects/altavision52/tmp_cie10_icd10_load.tsv
SET NAMES utf8mb4;

CREATE TABLE icd10_dx_order_code_full_backup_20260612_1025 LIKE icd10_dx_order_code;
INSERT INTO icd10_dx_order_code_full_backup_20260612_1025 SELECT * FROM icd10_dx_order_code;

ALTER TABLE icd10_dx_order_code MODIFY short_desc varchar(300) DEFAULT NULL;

DROP TEMPORARY TABLE IF EXISTS tmp_cie10_sync;
CREATE TEMPORARY TABLE tmp_cie10_sync (
  dx_code varchar(7) NOT NULL PRIMARY KEY,
...
SELECT COUNT(*) AS existing_matches_before_insert FROM icd10_dx_order_code d JOIN tmp_cie10_sync x ON x.dx_code = d.dx_code;
SELECT COUNT(*) AS missing_before_insert FROM tmp_cie10_sync x LEFT JOIN icd10_dx_order_code d ON d.dx_code = x.dx_code WHERE d.dx_code IS NULL;
SELECT COUNT(*) AS db_codes_not_in_excel_before_sync FROM icd10_dx_order_code d LEFT JOIN tmp_cie10_sync x ON x.dx_code = d.dx_code WHERE x.dx_code IS NULL;
...
LEFT JOIN icd10_dx_order_code d ON d.dx_code = x.dx_code
WHERE d.dx_code IS NULL;
...
SET
  d.formatted_dx_code = x.formatted_dx_code,
...
SELECT ROW_COUNT() AS matched_rows_touched_or_changed;
