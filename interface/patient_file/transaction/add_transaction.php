...
<div id="container_div" class="<?php echo $oemr_ui->oeContainer(); ?>">
    <form name='new_transaction' method='post' action='add_transaction.php?transid=<?php echo attr_url($transid); ?>'
                            ...
                            <div class='oe-pull-away' style='margin-right:25px;border-style:solid;border-width:1px;'>
                                <div style='margin:5px 5px 5px 5px;'>
                                    ...
                                    <?php if (!(empty($itemAMC)) && !(empty($itemAMC_elec))) { ?>
                                        &nbsp;&nbsp;<input type="checkbox" id="send_sum_elec_flag"
                                                           name="send_sum_elec_flag" checked>
                                    <?php } elseif (!(empty($itemAMC))) { ?>
                                        &nbsp;&nbsp;<input type="checkbox" id="send_sum_elec_flag"
                                                           name="send_sum_elec_flag">
                                    <?php } else { ?>
                                        &nbsp;&nbsp;<input type="checkbox" id="send_sum_elec_flag"
                                                           name="send_sum_elec_flag" disabled>
                                    <?php } ?>

                                    <span
                                            class="text"><?php echo xlt('Sent Summary of Care Electronically?') ?></span><br>
...
<script language="JavaScript">
    ...
    // Obtener el elemento del contenido donde se mostrará el resultado
    const contenidoElement = document.getElementById("contenido");

    // Función para cargar el contenido del enlace
    function cargarContenido() {
        // Realizar una solicitud HTTP GET al enlace
        fetch('<?php echo $GLOBALS['webroot']?>/interface/patient_file/summary/stats_full.php?active=all&category=medical_problem')
            .then(response => response.text())
            .then(data => {
                // Insertar el contenido en el elemento del contenido
                contenidoElement.innerHTML = data;
            })
            .catch(error => {
