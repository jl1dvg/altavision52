...
<div id="report_custom" style="width: 100%;">  <!-- large outer DIV -->
    ...
    if ($printable) {
        ...
        <?php echo text($facility['city']); ?>, <?php echo text($facility['state']); ?><?php echo text($facility['postal_code']); ?>
        <br clear='all'>
        <?php echo text($facility['phone']); ?><br>

        <a href="javascript:window.close();"><span
                class='title'><?php echo xlt('Patient') . ": " . text($titleres['fname']) . " " . text($titleres['lname']); ?></span></a>
        <br>
        ...
        <span><?php echo $_POST['pdf']; ?></span>
        <div class="report_search_bar" style="width: 100%;" id="search_options">
            <table style="width: 100%;">
                <tr>
                    <td>
                        <input type="text" onKeyUp="clear_last_visit();remove_mark_all();find_all();"
                               name="search_element" id="search_element" style="width: 180px;"/>
                    </td>
                    <td>
                        <a class="css_button"
                           onClick="clear_last_visit();remove_mark_all();find_all();"><span><?php echo xlt('Find'); ?></span></a>
                    </td>
                    <td>
                    ...
                    <td>
                        <input type="checkbox" onClick="clear_last_visit();remove_mark_all();find_all();"
                               name="search_case" id="search_case"/>
                    </td>
                    <td>
        ...
        if (stristr($key, "include_")) {
            ...
            if ($key == "documents") {
                ...
                if (($auth_notes_a || $auth_notes || $auth_coding_a || $auth_coding || $auth_med || $auth_relaxed)) {
                    ...
                    ?>
                    <div name="search_div" id="search_div_<?php echo attr($form_id) ?>_<?php echo attr($res[1]) ?>"
                         class="report_search_div class_<?php echo attr($res[1]); ?>">
                        <?php
...
<?php if (!$printable) { // Set up translated strings for use by interactive search ?>
    ...
    <script type="text/javascript"
            src="<?php echo $GLOBALS['web_root'] ?>/interface/patient_file/report/custom_report.js?v=<?php echo $v_js_includes; ?>"></script>
<?php } ?>
</body>
</html>
<?php } ?>
