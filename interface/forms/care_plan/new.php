...
$sql1 = "SELECT id AS `value`, lname, fname FROM `users` WHERE active = 1 AND authorized != ? ORDER BY lname";
$result = sqlStatement($sql1, array('0'));
...
<div class="container">
    ...
    <div class="row">
        ...
        <fieldset>
            <legend><?php echo xlt('Ingresar Detalles'); ?></legend>
            <?php
            if (!empty($check_res)) {
                        ...
                        <div class="form-group">
                            ...
                            <div class="forms col-xs-4">
                                <label for="description_<?php echo attr($key) + 1; ?>"
                                       class="h5"><?php echo xlt('Ojo'); ?>:</label>
                            ...
                            <div class="forms col-xs-2">
                                <label for="code_date_<?php echo attr($key) + 1; ?>"
                            ...
                            <div class="forms col-xs-2">
                                <label for="care_plan_type_<?php echo attr($key) + 1; ?>"
                                       class="h5"><?php echo xlt('Provider'); ?>:</label>
                                <select name="care_plan_type[]" id="care_plan_type_<?php echo attr($key) + 1; ?>"
                            ...
                            <div class="forms col-xs-1" style="padding-top:35px">
                                ...
                                <i class="fa fa-times-circle fa-2x text-danger" aria-hidden="true"
                                   onclick="deleteRow(this.parentElement.parentElement.parentElement.id);"
                                   title='<?php echo xla('Click here to delete the row'); ?>'></i>
                            </div>
                            <div class="clearfix"></div>
                            <input type="hidden" name="count[]" id="count_<?php echo attr($key) + 1; ?>" class="count"
                                   value="<?php echo attr($key) + 1; ?>">
                        </div>
                    </div>
                    <?php
                }
                ...
                <div class="tb_row" id="tb_row_1">
                    <div class="form-group">
                        ...
                        <div class="forms col-xs-2">
                            <label for="care_plan_type_1" class="h5"><?php echo xlt('Medico'); ?>:</label>
                            <select name="care_plan_type[]" id="care_plan_type_1" class="form-control care_plan_type">
                                ...
                                foreach ($care_plan_type as $value) {
                                    ...
                                    ?>
                                    <option
                                        value="<?php echo attr($value['fname'] . " " . $value['lname']); ?>" <?php echo $selected; ?>>
                                        <?php echo text($value['lname'] . " " . $value['fname']); ?>
                                    </option>
                                <?php } ?>
                            </select>
                        </div>
        ...
        <div class="form-group clearfix">
            <div class="col-sm-12 position-override">
                <div class="btn-group oe-opt-btn-group-pinch" role="group">
                    <button type="submit" onclick="top.restoreSession()"
                            class="btn btn-default btn-save"><?php echo xlt('Save'); ?></button>
                    <button type="button" class="btn btn-link btn-cancel oe-opt-btn-separate-left"
