...
<div id="container_div" class="<?php echo attr($oemr_ui->oeContainer()); ?>">
    ...
    <div class="row">
        <div class="col-sm-12">
            <form method="post" name="fee_sheet_form" id="fee_sheet_form"
                  action="<?php echo $rootdir; ?>/forms/fee_sheet/new.php?<?php
                ...
                <fieldset>
                    ...
                    <div class='col-xs-12 text-center table-responsive'>
                        <table name='selected_codes' id='selected_codes' class="table" cellspacing='5'>
                            <tr>
                                <?php // Checkbox para seleccionar todos los elementos
                                echo "<td colspan='10' class='billcell' align='right'><b>Eliminar todo</b><input type='checkbox' id='select-all' onclick='selectAll()'> </td>";; ?>
                            </tr>
                            <tr>
                                ...
                                <?php if (fees_are_used()) { ?>
                                    <?php if ($price_levels_are_used) { ?>
                                        <td class='billcell' align='center'><b><?php echo xlt('Price Level'); ?></b>&nbsp;
                                        </td>
                                    <?php } ?>
                ...
                <fieldset>
                    ...
                    <div class='col-xs-12'>
                        <div class="form-group col-lg-6 col-sm-12">
                            <label
                                class="control-label col-lg-4 col-sm-3 text-left"><?php echo xlt('Rendering'); ?></label>
                            <?php
                        ...
                        <div class="form-group col-lg-6 col-sm-12">
                            ...
                            if (!$GLOBALS['ippf_specific']) { ?>
                                <label
                                    class='control-label col-lg-4 col-sm-3 text-left'> <?php echo xlt('Supervising'); ?> </label>
                    ...
                    <div class="col-sm-12 position-override">
                        <div class="btn-group oe-opt-btn-group-pinch" role="group">
                            <button type='button' class='btn btn-default btn-calendar'
                                    onclick='newEvt()'><?php echo xlt('New Appointment'); ?></button>
                            <?php if (!$isBilled) { // visit is not yet billed ?>
                                <button type='submit' name='bn_refresh' class='btn btn-default btn-refresh'
                                        value='<?php echo xla('Refresh'); ?>'
                                        onclick='return this.clicked = true;'><?php echo xlt('Refresh'); ?></button>
                                <button type='submit' name='bn_save' class='btn btn-default btn-save'
                                        value='<?php echo xla('Save'); ?>'
                                    <?php
                                    if ($rapid_data_entry) {
                                        echo " style='background-color:#cc0000';color:#ffffff'";
                                    } ?>><?php echo xla('Save'); ?></button>
                                <button type='submit' name='bn_save_stay' class='btn btn-default btn-save'
                                        value='<?php echo xla('Save Current'); ?>'><?php echo xlt('Save Current'); ?></button>
                                <?php if ($GLOBALS['ippf_specific']) { // start ippf-only stuff ?>
                                    <?php if ($fs->hasCharges) { // unbilled with charges ?>
                                        <button type='submit' name='bn_save_close' class='btn btn-default btn-save'
                                                value='<?php echo xla('Save and Checkout'); ?>'><?php echo xlt('Save and Checkout'); ?></button>
                                    <?php } else { // unbilled with no charges ?>
                                        <button type='submit' name='bn_save_close' class='btn btn-default btn-save'
                                                value='<?php echo xla('Save and Close'); ?>'><?php echo xlt('Save and Close'); ?></button>
                                    <?php } // end no charges ?>
                                <?php } // end ippf-only ?>
                            <?php } else { // visit is billed ?>
                                <?php if ($fs->hasCharges) { // billed with charges ?>
                                    <button type='button' class='btn btn-default btn-show'
                                            ...
                                            echo "&ptid=" . attr_url($fs->pid) . "&enc=" . attr_url($fs->encounter); ?>'"
                                            value='<?php echo xla('Show Receipt'); ?>'><?php echo xlt('Show Receipt'); ?></button>
                                    <button type='submit' class='btn btn-default btn-undo' name='bn_reopen'
                                            onclick='return this.clicked = 2;'
                                ...
                                <button type='submit' class='btn btn-default btn-add' name='bn_addmore'
                                        onclick='return this.clicked = true;'
                            ...
                            <input type='hidden' name='form_has_charges'
                                   value='<?php echo $fs->hasCharges ? 1 : 0; ?>'/>
