...
<head>
    ...
    <style>
        ...
        div.titleheader {
            font-family: Arial;
            font-weight: bold;
            font-size: <?php echo round($FONTSIZE * 2.00); ?>pt;
            text-align: center;
            color: #000000;
            padding: 2pt;
            margin-bottom: <?php echo round($FONTSIZE * 0.22); ?>pt;
...
<form>
    ...
    //Datos del paciente
    $titleres = getPatientData($pid, "fname,lname, lname2, providerID,DATE_FORMAT(DOB,'%m/%d/%Y') as DOB_TS");
    //Fecha del Formulario
    $formfecha = sqlQuery(" SELECT `date`, `provider_id` FROM `forms` WHERE `pid` = $pid AND `form_id` = $formid AND `formdir` = '$formname'");
    $timestamp = strtotime($formfecha["date"]);
    ...
    $dmy = "Guayaquil,"." ".$numeroDia." de ".$nombreMes." del ".$anio;
    $operador = getProviderIniciales($formfecha['provider_id']);
    ...
    <?php //echo xlt('Date') ?>
        ...
        <p class='fecha'><?php echo $dmy;?></p><br><br><br />&nbsp;<br />
        <span class="subhead">
    <?php echo xlt('Patient') ?>: </span><span class="datospaciente"><?php echo text($titleres['fname']) . ' ' . text($titleres['lname']) . ' ' . text($titleres['lname2'])?></span> &nbsp;
        <span class='subhead'>
    <div class="titleheader">Informe de <?php echo str_replace("OCT","Tomografía con pruebas provocativas",$formtitle); ?><hr /></div>
</span>
    <?php } ?>
    ...
    ?>
    <footer><br><br><br><br><br><div class="firma"><span>
</span><br>Dr. Mario Pólit<br>
            Cirujano Oftalmólogo<br>
            Centro ALTAVISION</div>
        <br />
        <br />
        <br />
        <?php echo $operador; ?></footer>
</form>
<?php
if ($PDF_OUTPUT) {
    $content = getContent();
