...
<div id="container_div" class="<?php echo attr($oemr_ui->oeContainer()); ?>">
    ...
    <div class="row">
        <div class="col-sm-12">
            <form id="new-encounter-form" method='post' action="<?php echo $rootdir ?>/forms/newpatient/save.php"
                  name='new_encounter'>
                <?php if ($viewmode) { ?>
                ...
                <fieldset>
                    ...
                    <div id="visit-details">
                        <div class="form-group ">
                            <label for="pc_catid"
                                   class="control-label col-sm-2 oe-text-to-right"><?php echo xlt('Médico Tratante:'); ?></label>
                            <div class="col-sm-3">
                                <select class="form-control col-sm-12" name='tratante' id='provider_id'>
                                    <?php
                                    $ProviderSQL = "SELECT id, username, fname, lname FROM users WHERE
               authorized != 0 AND active = 1 ORDER BY lname, fname";

                                    $providerResult = sqlStatement($ProviderSQL);

                                    while ($urow = sqlFetchArray($providerResult)) {
                                        $tratanteID = $urow['id'];
                                        $tratanteNAME = $urow['lname'];
                                        $optionPRV = '<option value="%id%" %selected%>%lname%</option>';
                                        $optionPRV = str_replace("%id%", attr($tratanteID), $optionPRV);
                                        $optionPRV = str_replace("%lname%", text($urow['lname'] . " " . $urow['fname']), $optionPRV);
                                        if ($viewmode) {
                                            $selected = ($result['provider_id'] == $tratanteID) ? " selected" : "";
                                        }
                                        $optionPRV = str_replace("%selected%", $selected, $optionPRV);
                                        echo $optionPRV;
                                    }
                                    ?>
                                </select>
                            ...
                            <label for="pc_catid"
                                   class="control-label col-sm-2 oe-text-to-right"><?php echo xlt('Responsable:'); ?></label>
                            <div class="col-sm-3">
                                <select class="form-control col-sm-12" name='responsable' id='responsable_id'>
                                    <?php
                                    $ProviderSQL = "SELECT id, username, fname, lname FROM users WHERE
                authorized != 1 AND active = 1 ORDER BY lname, fname";
                                    ...
                                    while ($urow = sqlFetchArray($providerResult)) {
                                        $responsableID = $urow['id'];
                                        $responsableNAME = $urow['lname'];
                                        $optionPRV = '<option value="%id%" %selected%>%lname%</option>';
                                        $optionPRV = str_replace("%id%", attr($responsableID), $optionPRV);
                                        $optionPRV = str_replace("%lname%", text($urow['lname'] . " " . $urow['fname']), $optionPRV);
                                        if ($viewmode) {
                                            $selected = ($result['responsable_id'] == $responsableID) ? " selected" : "";
                                        }
                                        $optionPRV = str_replace("%selected%", $selected, $optionPRV);
                                        echo $optionPRV;
                                    }
                        ...
                        <div class="form-group ">
                            <label for="pc_catid"
                                   class="control-label col-sm-2 oe-text-to-right"><?php echo xlt('Visit Category:'); ?></label>
                            <div class="col-sm-3">
                            ...
                            ?>
                            <label for="pc_catid"
                                   class="control-label col-sm-2 oe-text-to-right"><?php echo xlt('Sensitivity:'); ?>
                                &nbsp;<i id='sensitivity-tooltip' class="fa fa-info-circle text-primary"
                        ...
                        <div class="form-group">
                            <label for='form_date'
                                   class="control-label col-sm-2 oe-text-to-right"><?php echo xlt('Date of Service:'); ?></label>
                            <div class="col-sm-3">
                                <input type='text' class='form-control datepicker col-sm-12' name='form_date'
                                       id='form_date' <?php echo $disabled ?>
                            ...
                            <div
                                ...
                                <label for='form_onset_date'
                                       class="control-label col-sm-2 oe-text-to-right"><?php echo xlt('Onset/hosp. date:'); ?>
                                    &nbsp;<i id='onset-tooltip' class="fa fa-info-circle text-primary"
                                             aria-hidden="true"></i></label>
                                <div class="col-sm-3">
                                    <input type='text' class='form-control datepicker col-sm-12' name='form_onset_date'
                                           id='form_onset_date'
                        ...
                        >
                            <label class="control-label col-sm-2 oe-text-to-right"><?php echo xlt('Referral Source'); ?>
                                :</label>
                            <div class="col-sm-3">
                        ...
                        <div class="form-group">
                            <label class="control-label col-sm-2 oe-text-to-right"><?php echo xlt('Datos de derivación'); ?>
                                :</label>
                            ...
                            <label class="control-label col-sm-2 oe-text-to-right"><?php echo xlt('Codigo de validación activo'); ?>
                                :</label>
                            <div class="col-sm-3">
                                <?php
                        ...
                        <?php if ($GLOBALS['enable_group_therapy']) { ?>
                            <div class="form-group" id="therapy_group_name" style="display: none">
                                <label for="form_group"
                                       class="control-label col-sm-2 oe-text-to-right"><?php echo xlt('Group name'); ?>
                                    :</label>
                                <div class="col-sm-3">
                                    <input type='text' name='form_group' class='form-control col-sm-12' id="form_group"
                                           ...
                                           readonly/>
                                    <input type='hidden' name='form_gid'
                                           value='<?php echo $viewmode && in_array($result['pc_catid'], $therapyGroupCategories) ? attr($result['external_id']) : '' ?>'/>
                                </div>
                                <div class="clearfix"></div>
                            </div>
                        <?php } ?>
                        <div class="form-group">
                            <label for='facility_id'
                                   class="control-label col-sm-2 oe-text-to-right"><?php echo xlt('Facility'); ?>
                                :</label>
                            <div class="col-sm-8">
                                <select name='facility_id' id='facility_id' class='form-control col-sm-9'
                                        onChange="bill_loc()">
                                    <?php
                                    if ($viewmode) {
                        ...
                        <div class="form-group">
                            <label for='billing_facility'
                                   class="control-label col-sm-2 oe-text-to-right"><?php echo xlt('Billing Facility'); ?>
                                :</label>
                    ...
                    <?php if ($GLOBALS['set_pos_code_encounter']) { ?>
                        <div class="form-group">
                            <label for='pos_code'
                                   class="control-label col-sm-2 oe-text-to-right"><?php echo xlt('POS Code'); ?>
                                :</label>
                            <div class="col-sm-8">
                    ...
                    <div class="form-group">
                        <div class="col-sm-10 col-sm-offset-1">
                            <textarea name="reason" id="reason" class="form-control" cols="80"
                                ...
                                } elseif (getReferral($pid, "body")) {
                                    echo text(getReferral($pid, "body"));
                                } else {
                                    echo text($GLOBALS['default_chief_complaint']);
                                }
                                ?> </textarea>
                        </div>
                    </div>
                            ...
                            <div class="form-group clearfix">
                                <div class="col-sm-6 col-lg-offset-3">
                                    <div class="col-sm-12">
                                            ...
                                            <div class="btn-group" role="group">
                                                <?php if (acl_check('patients', 'med', '', 'write')) { ?>
                                                    <a href="../../patient_file/summary/add_edit_issue.php"
                                                       class="css_button_small link_submit enc_issue"
                                        ...
                                        <div class="col-sm-12">
                                            <p>
                                                <i><?php echo xlt('To link this encounter/consult to an existing issue, click the '
                                                        . 'desired issue above to highlight it and then click [Save]. '
                                                        . 'Hold down [Ctrl] button to select multiple issues.'); ?></i>
                                            </p>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="form-group clearfix">
                    <div class="col-sm-12 text-left position-override">
                        <button type="button" class="btn btn-default btn-save"
                                onclick="top.restoreSession(); saveClicked(undefined);"><?php echo xlt('Save'); ?></button>
                        <?php if ($viewmode || empty($_GET["autoloaded"])) { // not creating new encounter ?>
                            <button type="button" class="btn btn-link btn-cancel btn-separate-left"
                                    onClick="return cancelClickedOld()"><?php echo xlt('Cancel'); ?></button>
                        <?php } else { // not $viewmode ?>
                            <button class="btn btn-link btn-cancel btn-separate-left link_submit"
...
<script language="javascript">
    ...
    var encounterDateField = document.getElementById('form_date');
    var referralEndDateField = document.getElementById('referral_end_date');
    var encounterDate = new Date();

    // Verificar la fecha al cargar la página
    checkReferralEndDate();

    referralEndDateField.addEventListener('change', checkReferralEndDate);

    function checkReferralEndDate() {
        var selectedDate = new Date(encounterDateField.value);

        if (selectedDate < referralEndDateField) {
            referralEndDateField.style.color = 'red';
        } else {
            referralEndDateField.style.color = '';
        }
    }
</script>
</html>
