...
if ($PDF_OUTPUT) {
    ...
    <table>
        ...
        if ($LMODSPH || $LMOSSPH) { ?>
            <tr style="border-bottom:1pt solid black;">
                <td style="font-weight:600;font-size:0.7em;text-align:right;"><?php echo xlt('Lensometría'); ?></td>
                <td style="font-weight:400;font-size:10px;text-align:center;"><?php echo xlt('OD{{right eye}}'); ?></td>
    ...
    if ($RBROW || $LBROW || $RUL || $LUL || $RLL || $LLL || $RMCT || $LMCT || $RADNEXA || $LADNEXA || $OSCONJ || $ODCONJ || $ODCORNEA || $OSCORNEA || $ODAC || $OSAC || $ODLENS || $OSLENS || $ODIRIS || $OSIRIS || $ODDISC || $OSDISC || $ODCUP || $OSCUP ||
        ...
        <p class="texto">Luego de realizar examen físico oftalmológico y fondo de ojo con oftalmoscopia indirecta con
            lupa
    ...
    if (!empty($orderDetails) || !empty($proceduresOD) || !empty($proceduresOI)) {
        ...
        <?php if (!empty($proceduresOD)) { ?>
            <p class="texto"><b><?php echo xlt('Procedimiento propuesto OD'); ?>
                    : </b><?php echo text(implode('. ', $proceduresOD)); ?>.</p>
        <?php } ?>
        <?php if (!empty($proceduresOI)) { ?>
            <p class="texto"><b><?php echo xlt('Procedimiento propuesto OI'); ?>
                    : </b><?php echo text(implode('. ', $proceduresOI)); ?>.</p>
        <?php } ?>
        <?php
    }
    ?>
    ...
    <P>
        <B>
            ...
            ?>
            <br>Centro Oftalmol&oacute;gico AltaVisi&oacute;n
            <br>
