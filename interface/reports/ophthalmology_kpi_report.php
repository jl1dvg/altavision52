...
function altavision_kpi_status_class($status)
{
    ...
    if ($status === 'temporary_zero') {
        return 'kpi-status-zero';
    }
...
<form method="post" name="theform" id="theform" action="ophthalmology_kpi_report.php" onsubmit="return top.restoreSession()">
    ...
    <div class="kpi-card-grid">
        ...
        <div class="kpi-card">
            ...
            <div class="kpi-card-note"><?php echo xlt('Pendientes reales, sin inventar valores'); ?></div>
            </div>
        <div class="kpi-card">
    ...
    <div class="kpi-layout">
        <div class="kpi-panel">
            <h3><?php echo xlt('Vista anual'); ?> - <?php echo text(altavision_kpi_segments()[$selected_segment]); ?></h3>
            <div class="kpi-scroll">
                <table class="table table-bordered table-striped kpi-table">
                    ...
                    <tr>
                        ...
                        <th class="kpi-long"><?php echo xlt('Formula'); ?></th>
                        <?php foreach (altavision_kpi_month_labels() as $monthLabel) { ?>
                            <th><?php echo text($monthLabel); ?></th>
                        <?php } ?>
                    </tr>
                    </thead>
                    <tbody>
                    <?php foreach ($rows as $row) { ?>
                        <tr>
                            ...
                            <td><?php echo text($row['original']); ?></td>
                            <td><?php echo text($row['formula']); ?></td>
                            <?php foreach (array_keys(altavision_kpi_month_labels()) as $monthNumber) { ?>
                                <td><?php echo text((string) altavision_kpi_template_month_value($row, $selected_segment, $monthNumber, $period)); ?></td>
                            <?php } ?>
                        </tr>
                    <?php } ?>
                    </tbody>
        ...
        <div class="kpi-panel kpi-actions">
            ...
            <ol>
                ...
                <li><?php echo xlt('Descargar Excel anual por IESS, Resto y Todos.'); ?></li>
            </ol>
            <hr>
            <h3><?php echo xlt('Notas'); ?></h3>
            <p><?php echo xlt('Los indicadores pendientes se muestran como N/A; los definidos operativamente en cero se presentan como 0 temporal v1.'); ?></p>
            <p><?php echo xlt('La tabla anual replica el formato operativo de los archivos base para facilitar correcciones.'); ?></p>
        </div>
    </div>

    <div class="kpi-layout" style="margin-top:14px;">
        <div class="kpi-panel">
