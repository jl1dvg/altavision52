<?php /* Smarty version 2.3.1, created on 2018-02-19 15:53:49
    ...
    foreach ($A_EVENTS as $date => $events) {
	    ...
	    $fname = substr($event['patient_name'], $commapos + 2);
            ...
            if ($catid == 2 || $catid == 3 || $catid == 4 || $catid == 8 || $catid == 11) {
                ...
                $divTitle .= "\n[".$atitle ."]";
                $content .= $catname;
                if ($event['recurrtype'] > 0) $content .= "<img src='$TPL_IMAGE_PATH/repeating8.png' border='0' style='margin:0px 2px 0px 2px;' title='".htmlspecialchars(xl("Repeating event"),ENT_QUOTES)."' alt='".htmlspecialchars(xl("Repeating event"),ENT_QUOTES)."'>";
